...
LIBPATH = os.path.join(PARENTDIR, "src", "release", "DenseTrackStab.so")
...
def densetrack(
    ...
    adjust_camera : bool
        Whether to adjust the camera motion with SURF
    ...
    The returned trajectories will have the following attributes:
        ...
        coords
            The 2D trajectory coordinates (2x[trajectory length + 1])
        ...
        displacement
            Only if adjust_camera=True. shape (2x[trajectory length])
    """
    ...
    data = _densetrack(
        ...
        poly_n, poly_sigma, image_pattern, adjust_camera
    )
